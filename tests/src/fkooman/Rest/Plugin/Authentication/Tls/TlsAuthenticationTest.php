...
';
    ...
    public function testTlsAuthCorrect()
    {
        ...
        $certInfo = $auth->isAuthenticated($request);
        $this->assertSame('__e3IYuUIKu_reNjy8ZHZ2PBh_H11eM5Rqs_KzxiHGg', $certInfo->getUserId());
    }

    public function testTlsAuthCorrectRedirectSslClientCert()
    {
        $request = new Request(
            array(
                'SERVER_NAME' => 'www.example.org',
     ...
     * @expectedExceptionMessage no_certificate
     */
    public function testTlsRequestAuthenticationNoCert()
    {
        $request = new Request(
            array(
                'SERVER_NAME' => 'www.example.org',
